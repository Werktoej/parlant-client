...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  return (
    <div className={`flex flex-col h-full bg-card overflow-hidden border border-border ${layoutMode === 'popup'
      ? 'rounded-lg shadow-2xl'
      : 'shadow-2xl'
      }`}>
      {/* Chat Header */}
      <div className="bg-primary p-4 sm:p-6 flex items-center justify-between relative overflow-hidden">

        <div className="flex items-center space-x-3 sm:space-x-4 relative z-10 min-w-0">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary-foreground rounded-lg flex items-center justify-center shadow-lg flex-shrink-0">
            <span className="text-primary font-bold text-lg sm:text-xl">F</span>
          </div>
          <div className="min-w-0 flex-1">
            <h3 className="text-primary-foreground font-bold text-lg sm:text-xl truncate">{displayAgentName}</h3>
            <div className="flex items-center gap-1.5 mt-0.5">
              <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
              <span className="text-primary-foreground/80 text-xs font-medium">{t('status.connected')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 relative z-10 flex-shrink-0">
          {layoutMode === 'fullscreen' && onToggleSessions && (
            <button
              ...
              className="hidden md:inline-flex items-center justify-center w-10 h-10 text-primary-foreground hover:text-primary-foreground hover:bg-primary-foreground/20 rounded-md transition-all duration-200 border border-primary-foreground/50"
          ...
          {layoutMode === 'popup' && onExpand && (
            <button
              onClick={onExpand}
              className="inline-flex items-center justify-center w-10 h-10 text-primary-foreground hover:text-primary-foreground hover:bg-primary-foreground/20 rounded-md transition-all duration-200 border border-primary-foreground/50"
          ...
          {onClose && (
            <button
              onClick={handleClose}
              className={`w-10 h-10 text-primary-foreground hover:text-primary-foreground hover:bg-primary-foreground/20 rounded-md transition-all duration-200 border border-primary-foreground/50 ${layoutMode === 'fullscreen' ? 'hidden md:inline-flex' : 'inline-flex'
